...
impl DeviceManager {
    ...
    fn add_ivshmem_device(
        ...
        let (region, mapping) = self.remaping_ram_region(
            start_addr,
    ...
    #[cfg(feature = "ivshmem")]
    pub fn remaping_ivshmem_bar_memory(&self) -> DeviceManagerResult<()> {
        if let Some(device) = self.ivshmem_device.as_ref() {
            ...
            let (new_region, new_mapping) = self.remaping_ram_region(
                new_addr,
